...
use jellyfish_verkle_tree::{
    ...
    verify_commitment_consistency, Commitment, FieldElement, Key, MemoryStore, NodeKey, Value,
};

/// Convenience: convert a single byte to a FieldElement value.
fn v(n: u8) -> FieldElement {
    value_to_field(&[n])
}
...
fn reject_proof_against_wrong_root() {
    ...
    insert(&mut store, &make_key(50, 0, 0), v(50));
...
fn reject_batch_with_one_wrong_value() {
    ...
    // All correct
    let correct: Vec<Option<Value>> = (0..5u8).map(|i| Some(v(i))).collect();
...
fn reject_batch_with_extra_key() {
    ...
    let values: Vec<Option<Value>> = (0..4u8).map(|i| Some(v(i))).collect();
    assert!(!verkle_proof::verify(
        &proof,
        root_c(&store),
        &more_keys,
...
fn single_key_tree() {
    let mut store = MemoryStore::new();
    insert(&mut store, &make_key(42, 0, 0), v(100));

    let rk = store.latest_root_key().unwrap();
    let proof = verkle_proof::prove_single(&store, &rk, &make_key(42, 0, 0)).unwrap();

    // Root is an EaS — no internal nodes, path = [extension, sub] = 2 commitments
...
fn zero_value() {
    let mut store = MemoryStore::new();
    insert(&mut store, &make_key(1, 0, 0), v(0)); // value is zero byte
    insert(&mut store, &make_key(2, 0, 0), value_to_field(&[])); // value from empty bytes
...
fn same_stem_different_suffixes() {
    // Many keys sharing the same stem (bytes 0..31) but different suffix (byte 31)
    let mut store = MemoryStore::new();
    for suffix in 0..10u8 {
        let mut key = [0u8; 32];
        key[0] = 1;
        key[31] = suffix;
        insert(&mut store, &key, v(suffix));
    }
    ...
    for suffix in 0..10u8 {
        ...
        key[0] = 1;
        key[31] = suffix;
        assert_eq!(get_committed_value(&store, &rk, &key), Some(v(suffix)));
    }
    ...
    let proof = verkle_proof::prove(&store, &rk, &keys).unwrap();
    let expected: Vec<Option<Value>> = (0..10u8).map(|s| Some(v(s))).collect();
    assert!(verkle_proof::verify(
        &proof,
        root_c(&store),
        &keys,
...
fn large_batch() {
    ...
    let expected: Vec<Option<Value>> = (0..50u8).map(|i| Some(v(i))).collect();
    assert!(verkle_proof::verify(
        &proof,
        root_c(&store),
        &keys,
...
fn commitment_consistency_after_many_operations() {
    ...
    for i in 0u8..30 {
        insert(&mut store, &make_key(i, 0, 0), v(i));
    }
    // Update some existing keys
    for i in 0u8..10 {
        insert(&mut store, &make_key(i, 0, 0), v(i + 100));
    }
    // Insert keys that cause splits at various depths
    for i in 0u8..10 {
        insert(&mut store, &make_key(0, i, 0), v(i + 200));
    }
    ...
    assert_eq!(
        get_committed_value(&store, &rk, &make_key(5, 0, 0)),
        Some(v(105))
    );
    assert_eq!(
        get_committed_value(&store, &rk, &make_key(15, 0, 0)),
        Some(v(15))
    );
    assert_eq!(
        get_committed_value(&store, &rk, &make_key(0, 5, 0)),
        Some(v(205))
    );
}
